...
def send_update(_table: Table,timer:int):
...
def update_status(_table:Table,timeract:int,timerinv:int,timerkill:int):
    while True:
        for route in _table.routes:
            if(route["Timer"]>=timeract and route["Timer"]<=timerinv):
                route["Status"]="Waiting"
            elif(route["Timer"]>=timerinv and route["Timer"]<=timerkill):
                route["Status"]="Invalid"
            elif(route["Timer"]>=timerkill):
